      ...
      async queryFn(id, _api, _extraOptions, baseQuery) {
        ...
        if (person.homeworld) {
          const homeworldData = await baseQuery({
            // on enlève la partie domain pour rester relatif
            url: person.homeworld.replace(apiBaseURL, ''),
          });
          result.homeworld = homeworldData.error ? 'Unknown' : homeworldData.data.name;
        } else {
        ...
        if (Array.isArray(person.vehicles) && person.vehicles.length) {
          const names = await Promise.all(
            person.vehicles.map(async (url) => {
              const vehicleData = await baseQuery({ url: url.replace(apiBaseURL, '') });
